...
public class WoodworksRecipeProvider extends BlueprintRecipeProvider implements IConditionBuilder {
	...
	public void buildRecipes(RecipeOutput output) {
		...
		ShapedRecipeBuilder.shaped(RecipeCategory.BUILDING_BLOCKS, Blocks.BOOKSHELF).define('#', ItemTags.PLANKS).define('X', Items.BOOK).pattern("###").pattern("XXX").pattern("###").unlockedBy("has_book", has(Items.BOOK)).save(output.withConditions(or(config(COMMON.woodenBookshelves, "wooden_bookshelves", true),config(COMMON.keepVanillaRecipes, "force_keep_tag_recipes"))));
		...
		ShapedRecipeBuilder.shaped(RecipeCategory.DECORATIONS, Blocks.LADDER, 3).define('#', Items.STICK).pattern("# #").pattern("###").pattern("# #").unlockedBy("has_stick", has(Items.STICK)).save(output.withConditions(or(config(COMMON.woodenLadders, "wooden_ladders", true),config(COMMON.keepVanillaRecipes, "force_keep_tag_recipes"))));
		ShapedRecipeBuilder.shaped(RecipeCategory.DECORATIONS, Blocks.BEEHIVE).define('P', ItemTags.PLANKS).define('H', Items.HONEYCOMB).pattern("PPP").pattern("HHH").pattern("PPP").unlockedBy("has_honeycomb", has(Items.HONEYCOMB)).save(output.withConditions(or(config(COMMON.woodenBeehives, "wooden_beehives", true),config(COMMON.keepVanillaRecipes, "force_keep_tag_recipes"))));
		ShapedRecipeBuilder.shaped(RecipeCategory.DECORATIONS, Blocks.CHEST).define('#', ItemTags.PLANKS).pattern("###").pattern("# #").pattern("###").unlockedBy("has_lots_of_items", CriteriaTriggers.INVENTORY_CHANGED.createCriterion(new InventoryChangeTrigger.TriggerInstance(Optional.empty(), new InventoryChangeTrigger.TriggerInstance.Slots(MinMaxBounds.Ints.atLeast(10), MinMaxBounds.Ints.ANY, MinMaxBounds.Ints.ANY), List.of()))).save(output.withConditions(or(config(COMMON.woodenChests, "wooden_chests", true),config(COMMON.keepVanillaRecipes, "force_keep_tag_recipes"))));

		ShapelessRecipeBuilder.shapeless(RecipeCategory.REDSTONE, Blocks.TRAPPED_CHEST).requires(Tags.Items.CHESTS_WOODEN).requires(Blocks.TRIPWIRE_HOOK).unlockedBy("has_tripwire_hook", has(Blocks.TRIPWIRE_HOOK)).save(output.withConditions(or(config(COMMON.woodenChests, "wooden_chests", true),config(COMMON.keepVanillaRecipes, "force_keep_tag_recipes"))));
		ShapelessRecipeBuilder.shapeless(RecipeCategory.REDSTONE, Blocks.TRAPPED_CHEST).requires(Blocks.CHEST).requires(Blocks.TRIPWIRE_HOOK).unlockedBy("has_tripwire_hook", has(Blocks.TRIPWIRE_HOOK)).save(output.withConditions(or(WOODEN_CHESTS,config(COMMON.keepVanillaRecipes, "force_keep_tag_recipes"))), Woodworks.location("trapped_chest"));
